...
> 本节从"麻雀版"给出 MARL 的骨架与结论；**完整展开见 [多智能体系统知识库 → 第 05 章 多智能体强化学习](../multi-agent-systems/05-marl.md)**（含 IGM 单调性证明、QMIX 损失推导、COMA 反事实基线、MAPPO 的 GAE 改造、自博弈与种群训练）。这里只保留三个必须记住的结论，便于与其他章节衔接。

### 3.1 三个必须记住的结论

| # | 结论 | 为什么重要 |
|---|------|-----------|
| 1 | **非平稳性**：对手也在学，环境分布 $P(s'\mid s,a)$ 变成 $P(s'\mid s,a,\pi_{-i})$ | 单智能体的收敛定理全部失效，"不收敛"往往是博弈类型问题 |
| 2 | **信用分配**：团队奖励无法区分个体贡献，值分解与反事实基线是主要解法 | 决定合作型任务能否学出有效分工 |
| 3 | **CTDE**：训练期集中（可用全局信息），执行期分布（只用局部观测） | 目前工业界唯一被大规模验证的可用范式 |

### 3.2 MARL 算法家族一页速查

| 家族 | 代表 | 核心机制 | 适用 |
|------|------|---------|------|
| 独立学习 | IQL | 各自当单智能体学 | 势博弈、弱耦合 |
| 值分解 | VDN → QMIX → QTRAN → QPLEX | 联合 Q 分解为个体 Q + 单调混合网络（IGM 条件） | 离散动作合作任务 |
| 集中式 Critic | MADDPG / COMA / MAPPO | Critic 看全局，Actor 各自贪心执行 | 连续、离散均可 |
| 自博弈与种群 | AlphaStar / OpenAI Five | 对手来自历史快照池，追求不可被利用 | 零和、竞争 |

| 算法 | 策略类型 | 信用分配方式 | 集中式信息 | 参数共享 | 主要弱点 |
|------|---------|-------------|-----------|---------|---------|
| MADDPG | 确定性 | 各智能体独立 Critic | 全部观测+动作进 Critic | 否 | Critic 输入维度爆炸 |
| QMIX | 值函数 | 单调混合 $Q_{tot}$ | 全局状态进 Mixer | 否 | 单调性限制表达力 |
| MAPPO | 随机 | 共享价值函数 | 全局状态进 Critic | 是（+ID 区分） | 大规模下信用分配仍模糊 |

> **核心思想**：MARL 的全部困难都可以归结为一句话 —— **你的环境里有别人，而且别人也在学习**。CTDE 是应对它的主流范式：训练期允许"作弊"看全局，执行期各自只看局部观测，从而同时拿到联合学习的表达力与去中心化的可部署性。

> **在别处展开**：非平稳性的形式化与不收敛反例、IGM 条件与 QMIX 单调性充分性证明、COMA 反事实基线推导、MAPPO 的 GAE 多智能体改造、均值场近似、种群训练与 PFSP、训练不稳定诊断表 —— 见 [多智能体系统 · 第 05 章](../multi-agent-systems/05-marl.md)；与之配套的基准、评估协议与统计检验见 [第 09 章](../multi-agent-systems/09-tools-environments.md)。
...
> 本节抽出三个方向的数学骨架：多智能体（纳什均衡、minimax、势博弈）、
> 对齐（KL 约束、闭式解、DPO）、离线学习（OOD 高估、CQL 保守估计）。
> 共同语言是**约束优化与不动点**。

### 10.1 博弈论与收敛（摘要）

> 完整的定义、证明与数值实验见 [多智能体系统 · 第 03 章 博弈论基础](../multi-agent-systems/03-game-theory.md) 与 [第 05 章 §二 非平稳性](../multi-agent-systems/05-marl.md)。这里只保留与本章（对齐、离线）直接相关的两条。

**纳什均衡**。策略组合 π = (π₁, …, π_N) 是均衡，若每个智能体在他人策略固定时都无法通过单方面偏离提高自己的期望回报：

$$\forall i,\ \pi_i' : \quad J_i(\pi_i, \pi_{-i}) \ \ge\ J_i(\pi_i', \pi_{-i})$$

有限博弈的混合策略纳什均衡**必然存在**（Nash 1950，Kakutani 不动点证得），但计算是 PPAD-完全的；求解算法（支撑枚举、虚拟对局、Lemke-Howson、CFR）见第 03 章 §五。

**非平稳性**。即使环境本身静止，只要他人策略在更新，从单个智能体视角看环境就是时变的：$P(s' \mid s, a)$ 变成 $P(s' \mid s, a, \pi_{-i})$。在势博弈等结构假设下，独立学习的动力学等价于对某个势函数爬山，因此收敛；在一般和博弈里，独立梯度上升可能进入极限环（石头剪刀布是最小反例）。**CTDE（见[第 05 章](../multi-agent-systems/05-marl.md)）之所以有效，是因为集中式 Critic 把 $\pi_{-i}$ 作为条件输入，把"漂移的环境"重新变成了"静止的输入"。**

> **核心思想**：非平稳性把"学习"变成"追逐移动目标"；没有对他人策略的建模或博弈结构假设，独立学习只能保证"不静止"，不能保证"收敛"。

继续深入：均衡的存在性与算法 → 第 03 章；独立学习的收敛条件与不收敛反例 → 第 05 章 §二；把"自利变成利他"的机制设计 → 第 04 章。
### 10.2 RLHF 的 KL 约束：从目标函数到拉格朗日对偶

**带 KL 惩罚的优化目标**。在奖励模型 r_θ 上做带正则的最大化：

$$\max_{\pi_\phi} \; \mathbb{E}_{x \sim D,\, y \sim \pi_\phi(\cdot \mid x)}\big[ r_\theta(x, y) \big] - \beta \cdot \mathrm{KL}\big( \pi_\phi(\cdot \mid x) \,\|\, \pi_{\mathrm{ref}}(\cdot \mid x) \big)$$

第一项奖励最大化，第二项惩罚偏离参考策略 π_ref（通常是 SFT 模型）。
奖励模型是**有偏的近似**：纯奖励最大化会让策略钻近似误差的空子，生成
"高分但违背人类意图"的文本（谄媚、冗长），即奖励黑客（reward hacking）；
KL 项把策略钉在 π_ref 附近，只在"可信邻域"内优化，并保留预训练的通用
能力。

**受约束 RL 视角**。KL 惩罚是软化的约束：严格版要求 KL(π_ϕ ‖ π_ref) ≤ ε
（受约束 MDP）。引入拉格朗日乘子 β ≥ 0：

$$\mathcal{L}(\pi_\phi, \beta) = \mathbb{E}[r_\theta] - \beta\big( \mathrm{KL}(\pi_\phi \| \pi_{\mathrm{ref}}) - \epsilon \big)$$

对固定 β 最大化 L 即带 KL 惩罚的 RLHF 目标；外层对 β 做对偶上升（约束
不满足就加大 β）即 PPO-Lagrangian。故 **KL 惩罚 = 约束的拉格朗日松弛**：
β 是"信任度"的价格——β 太小则奖励黑客，太大则退化为 π_ref（实践中需
动态调节）。
...
┌──────────────────────────────────────────────┐
│ 阶段一: 偏好 → 奖励                           │
│   (y_w ≻ y_l) ──Bradley-Terry──► r_θ         │
│   损失: -log σ(r_θ(x,y_w) - r_θ(x,y_l))      │
│ 阶段二: 奖励 → 策略                           │
│   max E[r_θ] - β·KL(π_ϕ ‖ π_ref)             │
│   ⇕ 拉格朗日对偶 ⇔ s.t. KL(π_ϕ‖π_ref) ≤ ε   │
└──────────────────────────────────────────────┘
```

> **核心思想**：KL 项不是工程补丁而是目标函数的必要组成——它在优化一个
> 近似奖励时划定了可信区域；等价地，它是 CMDP 约束的拉格朗日对偶形式。

小结：RLHF 目标 = 奖励最大化 + KL 正则；KL 正则防止奖励黑客，其本质是
约束 KL ≤ ε 的拉格朗日松弛，β 即对偶变量。

### 10.3 DPO：闭式解与奖励模型的消除

**闭式解**。对 10.3 的目标逐 x 求解，令 Z(x) = Σ_y π_ref(y|x)·e^{r_θ(x,y)/β}，
目标可重写为 −β·KL(π_ϕ ‖ (1/Z)·π_ref·e^{r_θ/β}) + β·log Z；KL 在 π_ϕ
等于目标分布时取零，故最优策略为

$$\pi^*(y \mid x) = \frac{1}{Z(x)}\, \pi_{\mathrm{ref}}(y \mid x)\, e^{r_\theta(x,y)/\beta}, \quad \text{即 } \pi^* \propto \pi_{\mathrm{ref}} \cdot e^{r/\beta}$$

**消去奖励模型**。反解 r_θ = β·log(π*/π_ref) + β·log Z，代入 Bradley-
Terry 偏好概率（§4.3），同一 (x, y_w, y_l) 对中的 Z(x) 相消；用参数化
策略 π_ϕ 替换 π* 并取负对数似然，即得 DPO 损失：

$$\mathcal{L}_{\mathrm{DPO}}(\phi) = - \mathbb{E}_{(x, y_w, y_l) \sim D}\Big[ \log \sigma\Big( \beta \log \frac{\pi_\phi(y_w \mid x)}{\pi_{\mathrm{ref}}(y_w \mid x)} - \beta \log \frac{\pi_\phi(y_l \mid x)}{\pi_{\mathrm{ref}}(y_l \mid x)} \Big) \Big]$$

**等价性**。推导只用两个假设：① RLHF 目标为"奖励 + KL"形式；② 偏好
服从 Bradley-Terry。两阶段 RLHF 隐式优化的正是 π*，DPO 以隐式奖励
β·log(π_ϕ/π_ref) 直接拟合偏好——二者在"最优策略"意义下等价；但 DPO
无需奖励模型、无需在线采样，把 RL 变成**偏好对上的二分类（logistic）
问题**。

```python
# DPO 损失计算: 单条偏好对 (y_w, y_l) 上
import numpy as np
beta = 0.1
logp_w, logp_l = -1.2, -3.5   # π_ϕ 对数概率(偏好者更高)
logr_w, logr_l = -2.0, -2.1   # π_ref 对数概率(接近)
logits = beta * ((logp_w - logr_w) - (logp_l - logr_l))
loss = -np.log(1.0 / (1.0 + np.exp(-logits)))
print("隐式奖励差 β·Δ =", round(float(logits), 4))
# 预期输出: 隐式奖励差 β·Δ = 0.22
print("DPO 损失 =", round(float(loss), 4))
# 预期输出: DPO 损失 = 0.5892
```

> **核心思想**：DPO 的成立依赖于"RLHF 目标有闭式解"；闭式解把奖励重新
> 参数化为策略比值，配分函数在偏好比较中消去，奖励模型被整体消除，
> RLHF 退化为监督式二分类。

小结：从 KL 约束目标解出 π* ∝ π_ref·e^{r/β}，反解奖励代入 Bradley-Terry
即得 DPO 损失；等价性来自"同一最优策略"这一共同不动点，DPO 免去了奖励
模型与在线 RL 的工程开销。

### 10.4 离线 RL 的分布外问题：高估机制与保守估计

**为什么 Q 会被高估**。离线数据 D 只覆盖行为策略 π_β 访问过的 (s, a)，
而 Q 学习目标含 max_{a'} Q(s', a')：argmax 很可能落在**数据中没有样本
的动作**上——这些 OOD 动作的 Q 从未被真实奖励或 TD 目标监督，只能无
约束外推（可任意大）。于是 Q 对 OOD 动作系统性高估，策略
π(s) = argmax_a Q(s,a) 被推向数据分布之外，那里高估最严重——**策略越
优化，估计越不可靠**（外推误差，Fujimoto et al. 2019）。在线 RL 中策略
会真的访问 argmax_a Q 并收到真实反馈纠正误差；离线时永远没有纠正信号。

**保守估计：CQL 的数学形式**。CQL 在贝尔曼误差上叠加一项，压低 OOD
动作的 Q、抬高数据内动作的 Q：

$$\mathcal{L}_{\mathrm{CQL}}(\theta) = \underbrace{\alpha\, \mathbb{E}_{s \sim D}\big[ \log \textstyle\sum_a e^{Q_\theta(s,a)} \big]}_{\text{压低所有动作(含 OOD)的 Q}}
- \underbrace{\alpha\, \mathbb{E}_{(s,a) \sim D}\big[ Q_\theta(s,a) \big]}_{\text{抬高数据内动作的 Q}}
+ \underbrace{\mathbb{E}_{D}\big[ \big(Q_\theta(s,a) - (r + \gamma \max_{a'} Q_{\bar\theta})\big)^2 \big]}_{\text{标准贝尔曼误差}}$$

第一项 log Σ_a e^Q 以 soft-max 形式压低全部动作（含 OOD）的 Q——正是
max_a Q 会选中的高 Q 动作；第二项防止数据内动作被过度压低。二者使 Q
成为**数据分布内价值的保守下界**（Q_θ ≤ Q*），等价于在"奖励不确定"下
取最坏情况估计。

**分布约束而非行为克隆**。行为克隆只学数据里出现过的动作，确实不踩 OOD
区域，但只复刻 π_β 的平均行为，丢弃了**价值信息**。离线 RL 的正确姿势
是**支持内利用**（extrapolation within support）：在数据分布内挑选更好
的动作（CQL 保守 Q、IQL expectile）。约束的是"对数据分布的偏离"，而非
策略本身。

> **核心思想**：离线 RL 的一切困难源于"max 操作遇到无监督的 OOD 动作"；
> 解法分两派——压制 OOD 的价值（CQL）或根本不查询（IQL）。共同原则是
> 分布约束：可以优化，但只能在数据覆盖的区域内优化。

小结：OOD 动作的 Q 无监督导致系统性高估，策略被推向数据外；CQL 用
soft-max 惩罚项压低未见动作的 Q 得到保守下界；离线 RL 的精髓是在数据
分布内利用价值信息，而非行为克隆。

### 10.5 理论小结：三个方向的统一视角

三个方向是同一枚硬币的三面——**优化目标与约束的博弈**：

| 维度 | MARL（博弈论） | RLHF/DPO（对齐） | Offline RL（离线学习） |
|------|--------------|----------------|---------------------|
| 优化对象 | 联合策略 (π₁,…,π_N) | 单策略 π_ϕ | 单策略 π_θ |
| 目标函数 | 各自收益 Vᵢ | E[r_θ] − β·KL | 贝尔曼误差 + 保守项 |
| 约束/正则 | 均衡（无单方偏离动机） | KL(π_ϕ‖π_ref) ≤ ε | 支持约束（不查询 OOD） |
| 失败模式 | 非平稳震荡、均衡低效 | 奖励黑客、能力遗忘 | OOD 外推高估 |
| 核心定理 | minimax、势博弈收敛 | 闭式解 π* ∝ π_ref·e^{r/β} | 外推误差、保守下界 |
| 统一语言 | 不动点（纳什均衡） | 带约束优化（拉格朗日对偶） | 分布偏移下的最坏情况估计 |

**统一视角**：三者的本质都是"在约束下优化"：MARL 的约束来自其他智能体
的理性（均衡），RLHF 来自参考策略（信任区域），Offline RL 来自数据
分布（支持区域）。约束过松出现失败模式（震荡、黑客、高估），过紧则退化
为次优（均衡低效、克隆 π_ref、行为克隆）。数学工具的价值正在于精确定义
"约束应该多紧"。

> **核心思想**：均衡/信任区域/支持区域都是"可行域"的不同形态；理解
> 约束的来源与松紧，就掌握了这三个方向的推演逻辑。
