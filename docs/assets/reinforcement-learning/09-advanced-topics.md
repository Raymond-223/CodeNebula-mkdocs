...
### 10.1 博弈论基础：均衡、博弈分类与收敛

**纳什均衡**。策略组合 π = (π₁, …, π_N) 是均衡，若每个智能体在他人策略
π₋ᵢ 固定时无单方偏离动机：

$$\forall i,\ \forall \pi_i': \quad V_i(\pi_i, \pi_{-i}) \ge V_i(\pi_i', \pi_{-i})$$

即 πᵢ 是 π₋ᵢ 的最优响应。均衡是联合策略上的**不动点**而非全局最优，
且可能有多重均衡。

```
纳什均衡示意（双人收益矩阵, 行=玩家1, 列=玩家2）
           玩家2
         C      D
玩家1 C (3,3) (0,5)
      D (5,0) (1,1)
(D,D) 是均衡: 任一方单方改到 C 收益从 1 降到 0;
(C,C) 不是均衡: 玩家1 单方改到 D 收益从 3 升到 5。
```
...
| 类别 | 收益结构 | 均衡性质 | 典型例子 |
|------|---------|---------|---------|
| 合作 | r₁ = … = r_N | 均衡 = 团队最优 | 多机器人搬运 |
| 竞争（零和） | r₁ + r₂ = 0 | 均衡 = 鞍点，价值唯一 | 双人棋、self-play |
| 混合 | 一般收益 | 均衡不唯一、可能低效 | 囚徒困境、竞价 |

**Minimax 定理**（von Neumann 1928）。两人零和有限博弈：

$$\max_{\pi_1} \min_{\pi_2} V(\pi_1, \pi_2) = \min_{\pi_2} \max_{\pi_1} V(\pi_1, \pi_2) = V^*$$

max/min 可交换，均衡值 V* 唯一——对抗训练收敛的基石：零和博弈的鞍点
即纳什均衡，且不依赖收敛路径。

**势博弈**（potential game）。若存在全局势函数 Φ 使任一智能体单方改
动作的收益变化等于 Φ 的变化：

$$\Phi(a_i', a_{-i}) - \Phi(a_i, a_{-i}) = r_i(a_i', a_{-i}) - r_i(a_i, a_{-i})$$

则"每人做最优响应"等价于"共同最大化 Φ"，退化为单目标协同优化；Φ 有
上界时梯度上升**必然收敛到纳什均衡**（合作博弈是平凡势博弈，取 Φ = r）。

> **核心思想**：博弈论回答"收敛到哪"——纳什均衡；但只有特殊结构（零和、
> 势博弈）才保证"能收敛"，一般混合博弈既无唯一解也无收敛保证。

小结：纳什均衡是 MARL 的解概念；零和博弈靠 minimax 定理保证鞍点存在，
势博弈靠势函数把多智能体优化还原为单目标优化。

### 10.2 独立学习与非平稳性：为什么单智能体算法会震荡

**非平稳性**。智能体 i 的"环境"包含其他智能体的策略 π₋ᵢ，而它们也在
学习，于是转移分布随 π₋ᵢ 漂移：

$$P(s' \mid s, a_i) = \sum_{a_{-i}} P(s' \mid s, a_i, a_{-i})\, \pi_{-i}(a_{-i} \mid s)$$

经验回放假设"过去样本仍代表当前分布"、贝尔曼不动点假设"环境固定"——
两者都被破坏，独立 Q 学习在双人博弈中甚至可能发散（有已知反例）。

**石头剪刀布：梯度上升的极限环**。双方各自对零和收益做投影梯度上升，
绕着均匀纳什均衡转圈、永不收敛：

```python
# 独立学习在 RPS 上不收敛: 零和收益, 双方投影梯度上升
import numpy as np
R = np.array([[0, -1, 1], [1, 0, -1], [-1, 1, 0]])  # 行=剪刀/石头/布

def proj(v):                        # 欧氏投影到概率单纯形
    u = np.sort(v)[::-1]
    c = np.cumsum(u) - 1
    k = np.nonzero(u - c / np.arange(1, len(v) + 1) > 0)[0][-1]
    return np.maximum(v - c[k] / (k + 1), 0)

lr = 0.1
p, q = np.array([.5, .3, .2]), np.array([.3, .5, .2])
D = []
for t in range(2000):
    p = proj(p + lr * (R @ q))      # 玩家1: 梯度上升
    q = proj(q - lr * (R.T @ p))    # 玩家2: 零和对偶
    if t % 250 == 0:
        D.append(np.linalg.norm(p - 1/3))
print("到均匀NE的距离:", np.round(D, 3))
# 预期输出: 到均匀NE的距离: [0.179 0.28  0.276 0.168 0.024 0.177 0.28  0.277]
```

RPS 无纯策略均衡，唯一纳什均衡是均匀混合策略；但均匀点附近的收益梯度
构成旋转场（线性化特征值模长大于 1），梯度上升被单纯形边界弹回形成
极限环——距离不归零即不收敛。这也是 CTDE（§3.3）有效的原因：集中式
Critic 把 π₋ᵢ 作为条件输入，吸收了非平稳性。

> **核心思想**：非平稳性把"学习"变成"追逐移动目标"；没有对他人策略的
> 建模或博弈结构假设，独立学习只能保证"不静止"，不能保证"收敛"。

小结：多智能体的环境因他人学习而漂移；RPS 演示了梯度上升的极限环行为，
独立学习需要结构假设或集中式信息才能收敛。

### 10.3 RLHF 的 KL 约束：从目标函数到拉格朗日对偶
