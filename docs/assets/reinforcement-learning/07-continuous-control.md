...
![连续控制算法架构](../img/07-continuous-arch.png)
...
![样本效率对比曲线](../img/07-sample-efficiency.png)
...
> 前九节给出算法与实现，本节补齐数学证明：SPG 为何在连续空间低效、DPG 定理从何而来、TD3 与 SAC 的设计各自针对哪个理论缺陷。记号沿用前文。

### 10.1 随机策略梯度在连续空间失效：采样方差爆炸

随机策略梯度定理形式上对动作空间无限制：

```
∇_θ J(π_θ) = E_{s∼ρ^π, a∼π_θ}[ ∇_θ log π_θ(a|s) · Q^π(s,a) ]
```

但对单个状态 s，内层是**动作积分** ∫_A π(a|s)·∇_θ log π(a|s)·Q(s,a) da，连续空间下只能蒙特卡洛估计。高斯策略 π(a|s) = N(a; μ_θ(s), σ²I) 的得分函数 ∇_θ log π = (a−μ)/σ²·∇_θ μ，估计量方差 Var[ĝ] ∝ d/(N·σ⁴)：收敛后期 σ→0 时按 σ⁻⁴ 爆炸，动作维度 d 大（机械臂 6~17 维）时噪声叠加，d 维盒子上的 MC 积分还需指数级样本。**确定性策略的直觉**：部署时只执行一个动作，何必维护整个分布？令 a = μ_θ(s)，梯度经链式法则化为 ∇_a Q·∇_θ μ_θ——动作积分消失，方差从"对动作采样"降为"对状态采样"。

> **核心思想**：SPG 的方差根源是"对动作分布求期望"这层 MC 采样；确定性策略删掉该积分，代价是探索必须外置。

**本节小结**：SPG 在连续空间并非不收敛，而是方差随 1/σ⁴ 与 d 爆炸；删除动作积分是唯一出路。

### 10.2 DPG 定理推导：随机策略梯度的确定性极限

Silver et al. (2014) 的 DPG 定理：

```
∇_θ J(μ_θ) = E_{s∼ρ^μ}[ ∇_a Q^μ(s,a)|_{a=μ_θ(s)} · ∇_θ μ_θ(s) ]
```

**推导（高斯收缩法）**：令 π_{μ,σ}(a|s) = N(a; μ_θ(s), σ²)，σ→0 时 π_{μ,σ} → δ(a−μ_θ(s))，ρ^{π_{μ,σ}} → ρ^μ。对 SPG 内层期望代入得分函数、换元 u=(a−μ)/σ，再对 Q 在 μ(s) 处一阶展开 Q^π(s, μ+σu) ≈ Q^π(s,μ) + σu·∇_a Q^π(s,μ)：

```
E_{a∼π}[ ∇_θ log π Q^π ] = (1/σ)·E_u[ u·Q^π(s, μ+σu) ] · ∇_θ μ_θ(s)
        ──σ→0──►  E_u[u·uᵀ]·∇_a Q^π(s,μ) = I·∇_a Q^μ(s,a)|_{a=μ(s)}      （E_u[u]=0，E_u[uuᵀ]=I）
```

取 σ→0 极限即得 DPG 定理。□ **与 SPG 的关系**：SPG 在"策略方差→0"时逐点收敛到 DPG——**DPG 是 SPG 的确定性极限**，有限 σ 相差 O(σ)；DPG 方差大降，代价是探索外置。

> **核心思想**：DPG 定理不是全新的梯度公式，而是随机策略梯度定理在方差趋于零时的极限，把"对动作积分求期望"替换为"代入单点求 ∇_a Q"。

**本节小结**：DPG 可由高斯策略的 SPG 经换元、泰勒展开、取极限三步严格导出，二者是同一梯度族的两端。

### 10.3 DDPG 的 off-policy 理论依据

off-policy 的核心障碍是重要性采样：随机策略的 TD 目标 r + γE_{a'∼π}[Q(s',a')] 须对动作求期望，用行为策略 β 的数据估计时要乘概率比 π/β，比值方差大且易爆炸。DPG 的梯度式中没有 log π 项、TD 目标退化为单点代入：

```
y = r + γQ^ω̄(s', μ_θ̄(s'))              # 无动作积分、无概率比
∇_θ J ≈ E_{s∼D}[ ∇_a Q^ω(s,a)|_{a=μ_θ(s)} · ∇_θ μ_θ(s) ]
```

行为策略 β(a|s) = μ_θ(s) + 噪声负责**探索**，目标策略 μ_θ 负责**利用**——数据由 β 生成、改进针对 μ，即 off-policy 标准结构。**经验回放**：确定性策略自身不探索，数据须靠外部噪声制造；回放池复用旧经验、打散时间相关性，样本效率提升一个数量级。**目标网络**：TD 目标依赖正在更新的 actor 与 critic，构成"自举+函数逼近+off-policy"致命三要素；冻结慢变副本 (ω̄, θ̄)（软更新 τ=0.005）使目标短期近似固定，恢复迭代稳定性。

> **核心思想**：确定性策略把 off-policy 的代价从"重要性采样"降为"目标偏离"（ρ^μ 与 ρ^β 的分布偏移），该偏移由回放池的广泛覆盖吸收——这是 DDPG 的工程权衡。

**本节小结**：DPG 梯度无概率比、TD 目标单点可算，是 off-policy 的数学前提；回放池与目标网络分别解决样本复用与致命三要素。

### 10.4 TD3 三项改进的理论动机

#### 10.4.1 clipped double-Q：用 min 抵消正偏差

max 算子对噪声是"凸"的：设 X, Y 为两个独立 critic 的估计误差（均值 0），则 E[max(X,Y)] ≥ max(E[X],E[Y]) = 0 而 E[min(X,Y)] ≤ min(E[X],E[Y]) = 0。取 max 系统性选出噪声正尾部——**高估**；取 min 系统性压低——**低估**。低估安全（策略宁可保守），高估则被 actor 放大：
...
Q 值
 ...
 │  估计 Q̂(s,·) ～ ～ ～ ～ ╱╲ ← 噪声抬出的虚假峰值
 └──────────────────────────────► a
      μ(s) 被推向虚假峰值 → 锁死过估计区 → 自举放大偏差
```

actor 最大化 Q̂，噪声峰值"看起来最好"，于是主动钻向过估计区域。数值验证：

```python
import numpy as np
rng = np.random.default_rng(0)
X, Y = rng.normal(0, 1, (2, 1_000_000))
print(f"E[max(X,Y)] = {np.maximum(X,Y).mean():.4f}    max(E[X],E[Y]) = {max(X.mean(),Y.mean()):.4f}")
print(f"E[min(X,Y)] = {np.minimum(X,Y).mean():.4f}    min(E[X],E[Y]) = {min(X.mean(),Y.mean()):.4f}")
```

预期输出：E[max(X,Y)] = 0.5642 > max(E[X],E[Y]) = 0.0004；E[min(X,Y)] = -0.5642 < min(E[X],E[Y]) = -0.0004。

TD3 解法：两个独立初始化的 critic 各自 TD 训练，目标取 min：y = r + γ·min(Q_{θ̄1}(s',ã'), Q_{θ̄2}(s',ã'))。误差近似独立，min 抵消正偏差（引入幅度相等的负偏差，方向安全）。

#### 10.4.2 延迟更新：让 critic 先收敛

actor 梯度 ∇_θ μ = ∇_a Q·∇_θ μ 要求 Q 在 μ(s) 邻域的梯度**可信**；critic 未收敛时 ∇_a Q 方向错误，actor 被带偏，又污染 critic 的 TD 目标，恶性循环。**两时间尺度原则**：价值更新快、策略更新慢，保证策略改进基于"近似收敛的 Q"。TD3 每 2 次 critic 更新才更新一次 actor 与目标网络（d=2），即该原则的离散实现。

#### 10.4.3 目标策略平滑：正则化 Q 的尖峰

Q̂(s,·) 在动作维上不平滑：actor 把动作推向最大化点，逼近在这些点产生尖峰。TD3 对目标动作加裁剪噪声 ã' = μ_θ̄(s') + ε，ε ~ clip(N(0,σ²), −c, c)，TD 目标对 ε 求期望等价于对 Q 做**邻域平均**：(1) 抹平虚假峰值；(2) 目标对 actor 估计误差不敏感（ε 充当扰动预算）；(3) 接近 expected-SARSA 的期望算子，方差低于 max 算子。

> **核心思想**：TD3 的三项改进是同一逻辑的三层递进——先承认 Q 会被高估（min 双 Q），再承认 Q 可能没学好（延迟更新），最后承认 Q 不平滑（目标平滑）。

**本节小结**：过估计源于 max 的凸性偏差并被 actor 放大；延迟更新落实两时间尺度原则；目标平滑是对 Q 的局部正则化。三者独立可叠加。
...
```
J(π) = E_{ρ_π}[ Σ_t γ^t ( r(s_t,a_t) + α·H(π(·|s_t)) ) ],   H(π(·|s)) = −E_{a∼π}[ log π(a|s) ]
```

定义软价值 V^π(s) = E_{a∼π}[Q^π(s,a) − α log π(a|s)]，Q^π(s,a) = r + γE_{s'}[V^π(s')]，消去 V 得软贝尔曼回溯：

```
Q^π(s,a) = r(s,a) + γE_{s',a'∼π}[ Q^π(s',a') − α log π(a'|s') ]
```

与标准 Q 回溯的唯一区别是目标中的 −α log π(a'|s')：低概率动作获得更高目标值，等于**显式奖励探索**。软贝尔曼算子 (T^π Q) = r + γE[Q − α log π] 是 γ-压缩映射，迭代 Q ← T^π Q 收敛到 Q^π——**软策略评估**（soft value iteration）。

#### 10.5.2 软策略改进

策略更新不是 argmax 硬切换，而是向"指数化 Q"的分布做 KL 投影：

```
π_new = argmin_{π'} E_{s∼D}[ D_KL( π'(·|s) ‖ exp(Q^{π_old}(s,·)/α)/Z(s) ) ]   （Z 为配分函数，可忽略）
```

展开 KL 并丢弃常数项，策略损失 L_π(φ) = E_{s∼D, a∼π_φ}[ α log π_φ(a|s) − Q^{π_old}(s,a) ]，闭式解是玻尔兹曼分布 π* ∝ exp(Q*/α)。软策略改进定理（Haarnoja et al., 2018）保证 Q^{π_new} ≥ Q^{π_old}，∀(s,a)；软评估与软改进交替执行，单调收敛到最大熵最优策略。

#### 10.5.3 重参数化：把采样变成可微路径

期望 E_{a∼π_φ} 仍有动作采样方差。SAC 用**重参数化技巧**消除：动作写成噪声的确定性函数 a = f_φ(ε;s) = tanh(μ_φ(s) + σ_φ(s)⊙ε)，ε ∼ N(0,I)，tanh 把动作压缩到 (−1,1) 满足边界约束。期望对 ε 重写后梯度穿过确定性路径：

```
∇_φ L_π = E_{ε∼N(0,I)}[ ∇_φ( α log π_φ(f_φ(ε;s)|s) − Q(s, f_φ(ε;s)) ) ]
```

这是"**期望内的梯度**"（低方差）；对比 REINFORCE 的"梯度内的期望"（得分函数，高方差）。tanh 挤压扭曲分布，log 概率需加雅可比修正：log π(a|s) = log N(u; μ,σ) − Σ_i log(1 − tanh²(u_i))，u = atanh(a)。

数值演示两种估计量的方差差异（f(a)=a²，真梯度 2μ）：
...
a = mu + eps
g_score   = eps * a**2      # 得分函数: (a-mu)/sigma^2 * f(a)
g_reparam = 2 * a           # 重参数化: nabla_a f(a) 沿可微路径
print(f"score   估计: 均值 {g_score.mean():.3f}  方差 {g_score.var():.1f}")
print(f"reparam 估计: 均值 {g_reparam.mean():.3f}  方差 {g_reparam.var():.1f}")
```

预期输出：score 均值 2.000 方差 30.1；reparam 均值 2.000 方差 4.0。两者皆无偏，重参数化方差低约一个数量级——这是 SAC 样本效率高于 REINFORCE 类方法的数学根源。

#### 10.5.4 熵项的探索与鲁棒性

高熵 = 动作分布宽，自动覆盖动作空间——**内建探索**，无需外部噪声；策略在近似最优动作间"对冲"，不依赖单一动作——**鲁棒性**；熵惩罚阻止锁死早期次优尖峰——**防过早收敛**（与 TD3 目标平滑异曲同工，但作用在策略分布上）。

SAC 整体结构（双 Q + 软更新 + 重参数化 actor）：

```
 环境 Env ─(s,a,r,s')─► 回放池 D ─mini-batch─► ┌────────────────────────────┐
                                                │ Critic Q_θ1, Q_θ2（双 Q）  │
                                                │ 目标: r + γ(min Q_θ̄ − αlogπ)│
                                                └─────────────┬──────────────┘
                                                ∇_a Q 沿重参数化路径回传 │
                                                ┌─────────────▼──────────────┐
                                                │ Actor: a = tanh(μ+σ⊙ε)    │
...
> **核心思想**：SAC 把"探索"从工程补丁（加噪声）升级为**目标函数的一部分**——熵项进入贝尔曼方程，价值、策略、探索统一在一个可证明收敛的框架里；重参数化再把梯度方差压到最低。

**本节小结**：软贝尔曼压缩映射保证评估收敛、KL 投影保证改进单调，重参数化消除动作采样方差，熵项同时提供探索与鲁棒性。

### 10.6 四方法理论对比表与小结

| 维度 | DPG (2014) | DDPG (2016) | TD3 (2018) | SAC (2018) |
|------|-----------|------------|-----------|-----------|
| 策略形式 | 确定性（线性） | 确定性（深度网络） | 确定性 | 随机（重参数化） |
| 目标函数 | 纯回报 | 纯回报 | 纯回报 | 回报 + α·熵 |
| 过估计处理 | 无 | 无 | clipped double-Q | min 双 Q（同 TD3） |
| 探索机制 | 外部噪声 | OU/高斯噪声 | 高斯噪声 | 熵正则 + 采样 |
| 目标网络 | 无 | 软更新 | 软更新 + 延迟更新 | 软更新 |
| 收敛特性 | 线性下有保证 | 易发散、需细调 | 稳定、不敏感 | 稳定、样本效率最高 |

理论缺陷 → 对策映射：积分不可行 → 确定性策略（DPG/DDPG/TD3）；采样方差爆炸 → 确定性策略/重参数化（DDPG/SAC）；max 凸性偏差 → clipped double-Q（TD3/SAC）；Q 未收敛 → 延迟更新（TD3）；Q 尖峰 → 目标平滑（TD3）；探索不足 → 熵正则（SAC）。

> **核心思想**：四条路线是同一理论问题的四种回答——DPG 定理解决"积分不可行"，TD3 的三个补丁解决"函数逼近引入的三个缺陷"，SAC 用熵正则把探索写进目标。

**本节小结**：DPG/DDPG/TD3/SAC 构成从"理论可行"到"工程稳健"的递进链条，各改进均可追溯到明确的数学缺陷；选型按"样本成本 × 稳定性需求"取舍。
