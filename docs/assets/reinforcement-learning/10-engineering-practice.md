...
> 前文给出"怎么排查"的工程清单，本节回答"为什么这些指标能指示收敛"：策略熵、TD 误差、KL 散度各自在数学上对应训练过程的什么状态。

### 10.1 策略熵：探索程度的定量标尺

策略熵 H(π(·|s)) = −Σ π(a|s) ln π(a|s) 度量策略的随机性：

- **熵高**：策略接近均匀分布，仍在广泛探索；
- **熵降**：策略逐渐确定性化，开始利用所学；
- **熵过低/归零**：策略塌缩到单一动作——若此时性能不佳，说明过早陷入局部最优（熵塌缩，entropy collapse）。
...
> **核心思想**：熵是探索-利用平衡的**可观测代理指标**；SAC 显式用熵正则项维持熵下限，PPO 则靠 clip 限制策略突变间接控熵。

### 10.2 TD 误差：预测一致性的度量

TD 误差 δ_t = r_{t+1} + γV(s_{t+1}) − V(s_t) 度量"相邻两步的价值预测是否自洽"。

当价值函数收敛到真实 v_π 时，TD 误差的期望趋近于零：

```
E[δ_t | S_t = s] = E[R_{t+1} + γv_π(S_{t+1}) | S_t=s] − v_π(s) = 0   （贝尔曼方程）
```

因此**TD 误差的均值→0 是价值收敛的必要信号**；但 TD 误差方差大并不代表发散（可能只是单步奖励噪声大），要结合滑动平均看趋势。

| 指标表现 | 含义 |
|---------|------|
| TD 误差均值缓慢趋零 | 价值函数在收敛 ✅ |
| TD 误差持续大且震荡 | 奖励尺度大/学习率过高，检查归一化 |
| TD 误差发散增长 | 不稳定（可能是 deadly triad），需降学习率/冻结目标网络 |

### 10.3 KL 散度：策略更新幅度的监控

KL(π_new ‖ π_old) = Σ π_new(a|s) ln(π_new(a|s)/π_old(a|s)) 度量一次更新前后策略的分布距离。

- KL 突增 → 更新步长过大，策略分布剧变，可能破坏已学内容；
- KL 长期接近 0 → 更新过小，训练停滞。

这正是 **TRPO 用 KL 约束、PPO 用 clip 限制**的原因：监控 KL 相当于直接检查"信任区域"约束是否被违反。

> **核心思想**：熵看探索、TD 误差看预测、KL 看更新幅度——三者构成训练过程的三维体检表。

### 10.4 收敛的理论充分条件

表格型方法（Tabular Q-Learning/TD）有严格收敛保证：
...
满足该条件的衰减步长（如 α_t = 1/t）保证表格 Q 以概率 1 收敛到 Q*。**函数逼近 + 自举 + off-policy 三者齐备时无此保证**（见第 5 章 deadly triad）——工程上的缓解手段（目标网络、回放、clip）本质上都是在"破坏"这些导致发散的组合条件。

### 10.5 理论判据速查表

| 信号 | 健康形态 | 危险形态 | 理论依据 |
|------|---------|---------|---------|
| 策略熵 | 缓慢下降、保持正值 | 骤降归零 | 探索-利用平衡 |
| TD 误差 | 均值趋零 | 持续大/发散 | 贝尔曼方程 |
| KL 散度 | 平稳小幅度 | 突增 | 信任区域 |
| 回报均值 | 单调上升后平台 | 剧烈震荡/骤降 | 随机逼近收敛 |
| 价值规模 | 有界 | 指数爆炸 | γ 收缩 |

**小结**：工程排查的每个信号背后都有理论依据——熵对应探索度量、TD 误差对应贝尔曼一致性、KL 对应策略更新幅度。理解了"为什么看这些指标"，调参就从试错变成了有指导的搜索。
