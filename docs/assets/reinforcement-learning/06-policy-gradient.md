...
> 正文第二节给出策略梯度定理的结论。本节补齐完整证明，并回答四个"为什么"：log 为何能把梯度搬进期望、期望形式为何能消掉转移概率、baseline 为何无偏、TRPO 为何需要 KL 约束。

### 10.1 log-derivative trick：把梯度搬进期望的恒等式

由链式法则 `∇_θ ln π = ∇_θ π / π`，两边同乘 `π_θ(a|s)`：

```text
∇_θ π_θ(a|s) = π_θ(a|s) · ∇_θ ln π_θ(a|s)          (10.1)
```

实质是**交换求导与求期望的顺序**。设 f(a) 与 θ 无关：

```text
∇_θ E_{a~π_θ}[f(a)] = ∇_θ Σ_a π_θ(a) f(a)
  = Σ_a [∇_θ π_θ(a)] f(a)            期望是有限和，求导逐项进行
  = Σ_a π_θ(a)[∇_θ ln π_θ(a)] f(a)   代入 (10.1)
  = E_{a~π_θ}[ f(a) ∇_θ ln π_θ(a) ]  (10.2)
```

关键是期望与求导可交换、f 不含 θ：对分布的梯度变成对数概率梯度的期望，可解析、可采样。取 f ≡ 1：

```text
E_{a~π_θ}[ ∇_θ ln π_θ(a) ] = Σ_a ∇_θ π_θ(a) = ∇_θ 1 = 0        (10.3)
```

> **核心思想**：(10.2) 把无法采样的 ∇E 换成可采样的 E[∇lnπ·f]；(10.3) 保证对数概率梯度期望为零。二者分别是策略梯度的引擎与消项、无偏性的钥匙。

**小结**：log-derivative trick 即 (10.1) 变形与 (10.3) 零期望性质，后续证明反复调用。

### 10.2 策略梯度定理的完整证明

**定理**：`J(θ) = E_{τ~π_θ}[Σ γ^t r_t]`，则

```text
∇_θ J(θ) = E[ Σ_t γ^t G_t ∇_θ ln π_θ(A_t|S_t) ]                (10.4)
```

其中 `G_t = Σ_{l≥0} γ^l r_{t+l}` 为折扣回报。

**第一步：目标函数展开。** 轨迹概率（μ、p 均与 θ 无关）：

```text
P_θ(τ) = μ(S_0) · Π_k π_θ(A_k|S_k) · p(S_{k+1}|S_k, A_k)
```

用期望线性性拆回报、用 score 技巧：

```text
∇_θ J(θ) = Σ_t γ^t E[ r_t ∇_θ ln P_θ(τ_{≤t}) ]
         = Σ_t γ^t E[ r_t Σ_{k≤t} ∇_θ ln π_θ(A_k|S_k) ]         (10.5)
```

μ、p 不含 θ，求导后消失——环境模型第一次被消掉。

**第二步：对每个时刻的贡献求梯度并重排求和。** 交换 (10.5) 中对 t、k 的求和：

```text
∇_θ J(θ) = E[ Σ_k γ^k G_k ∇_θ ln π_θ(A_k|S_k) ]                (10.6)
```

**第三步：用 E[∇lnπ] = 0 消去交叉项。** (10.5) 中"未来梯度 × 过去回报"交叉项必须为零。对 k > t，由塔性质与马尔可夫性：

```text
E[ r_t ∇_θ ln π_θ(A_k|S_k) ]
  = E[ r_t E[∇_θ ln π_θ(A_k|S_k)|S_k] ]       塔性质
  = E[ r_t Σ_a π_θ(a|S_k) ∇_θ ln π_θ(a|S_k) ] 条件期望只依赖当前状态
  = E[ r_t ∇_θ 1 ] = 0                         由 (10.3)
```

交叉项消失，(10.6) 成立，得证。∎

> **核心思想**：回报期望按时刻展开，每时刻梯度贡献是"前缀轨迹的 score"：θ 无关项被求导杀死，交叉项被 (10.3) 杀死，剩下的 `Σ γ^t G_t ∇lnπ` 即所求。

**小结**：证明 = 展开 (10.5) → 重排 (10.6) → 零期望消项；工具是 score、期望线性性、马尔可夫性。

### 10.3 为什么期望形式能消掉未知的转移概率

(10.4) 中**没有出现 p(s'|s,a) 的任何符号**，原因有二：

1. 求导层面：`∇_θ ln P_θ(τ)` 中 p 与 θ 无关，`∇_θ ln p = 0`；
2. 估计层面：期望对"环境实际产生的轨迹"求，我们只观测 `(S_t, A_t, r_t)`，从不计算 `P_θ(τ)`——转移概率藏在采样测度里隐式加权，在估计量中不露面。

这是策略梯度**能在 model-free 下工作的根本原因**：估计只需策略梯度与观测到的动作、回报，环境机制完全透明。

```text
                梯度流（数据流向图）

  θ ──→ π_θ(a|s) ──→ 采样 A_t ~ π_θ(·|S_t) ──→ 环境（黑箱，p 未知）
                                                      │
        ĝ = Σ_t γ^t G_t ∇_θ ln π_θ(A_t|S_t)  ←────────┘ 只观测 (S_t, A_t, r_t)
                  ↑
        θ ← θ + α·ĝ         全程不触碰 p(s'|s,a)
```

> **核心思想**：策略梯度"只需要策略的梯度，不需要环境的梯度"，这是它在环境未知、模拟器不可微时仍能工作的根本原因。

**小结**：转移概率从求导层（∇ln p = 0）与估计层（测度隐式加权）同时消失，故策略梯度天然 model-free。
...
```text
E_{a~π_θ(·|s)}[ ∇_θ ln π_θ(a|s) b(s) ]
  = b(s) Σ_a π_θ(a|s) ∇_θ ln π_θ(a|s)
  = b(s) ∇_θ Σ_a π_θ(a|s) = b(s) ∇_θ 1 = 0      由 (10.3)
```

对轨迹求期望（塔性质）得该项为零，故用 `G_t − b(S_t)` 替换 `G_t` 不改变梯度期望、只改变方差——**任何状态相关基线都无偏**。

**方差最小化与最优基线**。令 `d Var[(G_t − b(S_t))∇lnπ]/db = 0`：

```text
b*(s) = E[ G_t ||∇_θ ln π_θ(A_t|S_t)||² | S_t=s ] / E[ ||∇_θ ln π_θ(A_t|S_t)||² | S_t=s ]
```

即以 score 范数平方为权重的价值函数；score 范数近乎恒定时 `b*(s) ≈ V^π(s)`——这是"价值函数做基线"近乎最优的依据。

数值验证（softmax 策略，可直接运行）：
...
def grad_log_pi(theta, a):          # ∇_θ ln π_θ(a) = 独热向量 − 概率向量
    p = softmax(theta)
    g = np.zeros_like(theta); g[a] = 1.0
    return g - p

theta = rng.normal(size=4)
acts = rng.choice(4, size=200000, p=softmax(theta))
g = np.array([grad_log_pi(theta, a) for a in acts])
print(np.round(g.mean(axis=0), 4))            # ≈ [0 0 0 0]
print(np.round((g * 3.7).mean(axis=0), 4))    # ≈ [0 0 0 0]
```

预期输出：`[-0.0004  0.0005  0.002  -0.0022]` 与 `[-0.0014  0.002  0.0073 -0.008 ]`（≈ 0）。

> **核心思想**：baseline 之所以"免费"，是因为 (10.3) 保证了 score 与任何状态函数正交；方差分析进一步指出，最优基线正是以 score 范数为权的价值函数。

**小结**：baseline 无偏性来自 (10.3) 的逐状态零期望；最优基线是 score 加权的价值函数，`V(s)` 是其自然近似。

### 10.5 Advantage 与 GAE 的理论

**优势函数**。`A^π(s,a) = Q^π(s,a) − V^π(s)` 衡量"在 s 采取 a 相对平均水平好多少"。V 只依赖状态，正是 (10.4) 允许减去的 baseline，故 `∇J = E[Σ γ^t A^π(S_t,A_t) ∇lnπ(A_t|S_t)]`。

**GAE 构造**。定义 TD 残差 `δ_t = r_t + γV(S_{t+1}) − V(S_t)`，望远镜求和得 `A^π(s,a) = Σ_{l≥0} γ^l E[δ_{t+l}|S_t=s,A_t=a]`。截断到前 k 项得 k 步优势 `Â^(k)_t = Σ_{l=0}^{k-1} γ^l δ_{t+l}`：k=1 单步 TD（高偏差低方差），k→∞ 蒙特卡洛（低偏差高方差）。GAE 取 λ 几何加权平均：

```text
GAE(λ) = Σ_{l≥0} (γλ)^l δ_{t+l} = (1−λ) Σ_{k≥1} λ^(k−1) Â^(k)_t    (10.7)
```

**λ 是偏差-方差旋钮**：λ 越小偏差越大、方差越小；λ 越大反之。λ=0 退化为 δ_t，λ=1 退化为 `G_t − V(S_t)`；有效视野 `1/(1−γλ)`。
...
def compute_gae(rewards, values, gamma=0.9, lam=0.8):
    """rewards 长 T；values 长 T+1（末位 V(s_T) 自举），递推 (10.7)"""
    gae = np.zeros(len(rewards)); adv = 0.0
...
print(np.round(compute_gae(np.array([1.0, 0.0, 2.0, 0.5]),
                           np.array([0.0, 1.0, 0.5, 2.0, 1.0])), 4))
```

预期输出：`[ 2.9908  1.515   2.868  -0.6   ]`

> **核心思想**：GAE 用 λ 在"相信价值函数（低方差、有偏）"与"相信真实回报（无偏、高方差）"间连续插值，无需新样本，只是对同一批 (r, V) 数据做不同权重的后处理。

**小结**：A = Q − V 是无偏基线化优势；(10.7) 用几何权重合成 k 步优势估计族，λ 是偏差-方差旋钮。

### 10.6 TRPO 的约束优化理论

**vanilla PG 的信任区域问题**。`θ ← θ + η·ĝ` 的隐患：ĝ 含噪，η 稍大真实 J 即可能下降；且策略是概率分布，参数小扰动会被指数放大——一步过大导致**策略崩溃**（退化为确定性坏策略，训练不可逆失败）。

**理论支撑**。以旧策略采样的替代目标 `L_θ_old(θ) = E[ (π_θ/π_old) A^π_old ]` 在 θ_old 处与 J 一阶相等（∇L = ∇J），二阶以上有偏差。Kakade & Langford 给出下界：

```text
J(θ) ≥ L_θ_old(θ) − C · max_s KL(π_old(·|s) ‖ π_θ(·|s))          (10.8)
```

"最大化替代目标 + 压住 KL"即可保证 J 单调不降（minorize-maximization）。TRPO 写成约束优化：

```text
max_θ  E[ ρ_t(θ) Â_t ]           ρ_t(θ) = π_θ(A_t|S_t)/π_old(A_t|S_t)
s.t.   E_s[ KL(π_old(·|s) ‖ π_θ(·|s)) ] ≤ δ                       (10.9)
```

用自然梯度 `θ ← θ + α·F⁻¹∇L` 求解（F 为 Fisher 矩阵 ≈ KL 局部 Hessian），配共轭梯度与线搜索；F⁻¹ 把梯度"掰弯"到 KL 椭球内最陡方向。

```text
    信任区域示意图（θ 平面）

              J(θ) 等高线
        θ_old ──F⁻¹∇L──→ θ*     信任区域：以 θ_old 为中心的 KL 椭球 {KL ≤ δ}
          │                      （用 KL 度量分布变化，而非欧氏距离）
          ▼
        普通梯度 ∇L 一步越界 → 分布剧变 → J 骤降（策略崩溃）
```

**PPO：裁剪替代约束**。TRPO 每步解二阶系统代价高；PPO 观察到约束本质是"不允许 ρ_t 偏离 1 太远"，直接在目标上把 ρ 轴"焊死"：

```text
L^CLIP(θ) = E[ min( ρ_t(θ)Â_t,  clip(ρ_t(θ), 1−ε, 1+ε) Â_t ) ]      (10.10)
```

分段线性性质（Â 的符号决定哪一支起作用）：

| 情形 | ρ 轴上的形状 | 效果 |
|------|-------------|------|
| Â_t > 0（好动作） | ρ: 1→1+ε 线性上升，ρ>1+ε 被压平 | 鼓励适度增大 ρ，超过 1+ε 不再奖励 |
| Â_t < 0（坏动作） | ρ: 1→1−ε 线性下降，ρ<1−ε 被压平 | 惩罚过度提高坏动作概率，同样封顶 |

每次更新只有 clip 区间内一段被推着走，等效于每步只迈一小步的软信任区域，无需二阶信息。对比 TRPO：硬约束 + 自然梯度（二阶、开销高、有 (10.8) 保证）vs PPO：软约束 + Adam（一阶、开销低、实践稳定）。

> **核心思想**：TRPO/PPO 的动机是在同一份旧数据上做多次更新（off-policy 复用），必须限制新旧策略差异：TRPO 用 KL 硬约束 + 自然梯度，PPO 用裁剪把"离 1 太远的 ρ"的梯度清零，殊途同归。

**小结**：vanilla PG 大步更新会因分布剧变而崩溃；(10.8) 给出"替代目标 − KL 惩罚"下界，TRPO 据此做 KL 约束优化，PPO 用 (10.10) 的分段线性裁剪廉价近似。
