...
> 工具链解决了"怎么跑实验"，本节回答"怎么科学地读实验结果"：单次运行的奖励曲线为什么不可信？均值比较需要多少样本？答案藏在统计推断里。

### 10.1 单次运行的不可靠性

强化学习训练是**高度随机**的过程：环境转移、策略采样、回放缓冲抽样都引入噪声。单次运行的回报曲线是带噪声的过程实现，其逐点值几乎不携带信息。
...
**核心思想**：单次运行 = 一个样本；结论必须建立在**多次运行（多个随机种子）的均值与方差**上。

### 10.2 置信区间：均值估计的可靠性

假设用 N 个随机种子跑了 N 次实验，得到最终性能 x_1, ..., x_N，则真实均值 μ 的 (1−α) 置信区间为：

```
CI = x̄ ± t_{α/2, N-1} · s / √N
```

其中 x̄ 是样本均值，s 是样本标准差，t 是 Student-t 分布的分位数。**CI 越窄，结论越可信**；CI 越宽，说明实验噪声越大、需要更多种子。

| 种子数 N | t_{0.975,N-1} | CI 半宽（相对 s/√N 的倍数） |
|---------|--------------|---------------------------|
| 3 | 4.303 | 2.48 × s |
| 5 | 2.776 | 1.24 × s |
| 10 | 2.262 | 0.72 × s |
...
> **核心思想**：种子数从 3 加到 10，CI 缩窄约 3.4 倍——实验预算优先花在"多跑几个种子"上，而不是单次跑更长。

### 10.3 算法比较的假设检验

比较算法 A 与 B：不能直接比较均值大小，必须检验差异是否显著。配对设置下用配对 t 检验：

```
H0: μ_A = μ_B（两算法性能无差异）
t = (d̄) / (s_d / √N)   其中 d_i = x_A,i − x_B,i（同种子配对差）
```

**p 值 < 0.05** 才能宣称 A 优于 B。常见误区：

| 误区 | 正确做法 |
|------|---------|
| 比单次运行曲线高低 | 多次运行取均值±CI |
| 比最终数值大小 | 配对 t 检验 / Mann-Whitney U |
| 训练中途截断对比 | 固定预算（步数/交互数）再比较 |
| 只报最好种子 | 报告全部种子（含失败）的中位数 |

### 10.4 多重比较与曲线聚合

多算法同时对比（A/B/C/D）时，两两 t 检验会膨胀假阳性率（family-wise error），需 Bonferroni 校正：显著水平 α/m（m 为比较次数）。

曲线聚合的规范做法：**每个种子一条曲线 → 逐点取均值与 CI → 画 mean±CI 阴影带**。阴影带不重叠处才是可信的差异。

> **核心思想**：实验报告 = 均值曲线 + CI 阴影带 + 种子数 + 显著性检验；缺任何一项都不可复现。

### 10.5 可复现性的统计要求

| 要素 | 统计作用 |
|------|---------|
| 固定随机种子 | 消除初始化噪声，保证可复现 |
| 报告种子数 N | 决定 CI 宽度 |
| 报告超参与环境版本 | 消除配置漂移 |
| 报告均值±std | 给出效应量与不确定性 |

**小结**：评估的科学性 = 多次运行 + 置信区间 + 假设检验。工具链只能保证"能跑"，统计方法才能保证"结论可信"。
