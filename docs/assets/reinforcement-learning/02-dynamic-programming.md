...
![4×4 GridWorld 最优策略](../img/02-optimal-policy.png)
...
<!-- 理论增补：插入原文件「## 附：本章速查表」之后。记号沿用本章。 -->

## 十、理论进阶：策略迭代与价值迭代的收敛性

前八节给出算法与实验，本节补上理论骨架：为什么"评估—改进"循环必然收敛、收敛多快、误差如何控制。记号沿用本章：$\gamma\in[0,1)$ 为折扣因子（$\gamma=1$ 情形见 10.3），$\mathcal{T}_\pi$、$\mathcal{T}^*$ 为贝尔曼期望算子与最优算子。

### 10.1 策略改进定理（Policy Improvement Theorem）的证明

**定理**：设 $\pi$、$\pi'$ 为确定性策略。若对**所有** $s$ 有 $q_\pi\big(s,\pi'(s)\big) \ge v_\pi(s)$，则对所有 $s$ 有 $v_{\pi'}(s) \ge v_\pi(s)$；若某状态严格大于，则同一状态处 $v_{\pi'} > v_\pi$。

**证明**：对任意 $s$，把 $v_{\pi'}$ 按贝尔曼期望方程展开，每展开一层就用假设替换"下一步价值"，逐项比较：

```
  v_π(s) ≤ E_{π'}[ r_1 + γ v_π(S_1) | s ]                     ①
         ≤ E_{π'}[ r_1 + γ r_2 + γ² v_π(S_2) | s ]            ② ①在 S_1 处展开
         ≤ E_{π'}[ r_1 + γ r_2 + γ² r_3 + γ³ v_π(S_3) | s ]   ③ ②在 S_2 处展开
         ……
         ≤ E_{π'}[ Σ_{t=1..n} γ^{t-1} r_t + γ^n v_π(S_n) | s ]

令 n → ∞：γ<1 时 γ^n v_π(S_n) → 0（v_π 有界）；γ=1 的吸收任务中
v_π(S_n) → 0（几乎必然到达吸收态）。右侧收敛到 E_{π'}[Σ_{t≥1} γ^{t-1} r_t] = v_π'(s)，
故 v_π'(s) ≥ v_π(s) 对所有 s 成立；若①严格，用 v_π' ≥ v_π 代回①得
v_π'(s) ≥ q_π(s, π'(s)) > v_π(s)。□
```

**贪心改进自动满足假设**：当 $\pi' = \mathrm{greedy}(v_\pi)$ 时 $q_\pi\big(s,\pi'(s)\big) = \max_a q_\pi(s,a) \ge q_\pi\big(s,\pi(s)\big) = v_\pi(s)$。故只要做贪心改进，前提自动成立、改进永不倒退；$\pi' \ne \pi$ 时还有状态严格变好——3.2 节结论的严格化。

> **核心思想**：策略改进定理是策略迭代的"引擎"：贪心改进永不倒退，且"改进不动 ⇔ 已最优"。证明套路是"展开—替换—取极限"：每展开一层贝尔曼期望方程，就消费一次假设。

**小结**：定理把"逐状态动作价值比较"升级为"全局价值比较"；对贪心改进前提自动满足，故 PI 每轮都不退化。

### 10.2 策略迭代（PI）的收敛性

**单调性**：设 $\pi_0 \to \pi_1 \to \cdots$ 为 PI 的策略序列，$v_k = v_{\pi_k}$。由 10.1，$v_{k+1} \ge v_k$（分量wise）——价值序列单调非降。

**有限终止**：有限 MDP 中确定性策略总数 $\lvert\mathcal{A}\rvert^{\lvert\mathcal{S}\rvert}$ 有限。若 $\pi_{k+1} = \pi_k$，由 3.4 节判据 $v_{\pi_k} = v_*$，停机输出最优；若 $\pi_{k+1} \ne \pi_k$，则至少一个状态严格改进、价值向量严格变大，而同一策略不可能对应两个不同价值，故策略不可能重复。严格单调序列穿过有限集合，**至多 $\lvert\mathcal{A}\rvert^{\lvert\mathcal{S}\rvert}$ 轮必然终止**。

**轮数分析**：上界指数级、极松；实践通常 3–10 轮，因每轮是**全局**贪心，一次修正大量状态（GridWorld 4 轮、11 次扫描）。每轮代价 = 评估 $O(k\lvert\mathcal{S}\rvert^2\lvert\mathcal{A}\rvert)$（$k$ 次扫描）+ 改进 $O(\lvert\mathcal{S}\rvert\lvert\mathcal{A}\rvert)$。

**数值验证（可运行）**：自包含随机 MDP（$\lvert\mathcal{S}\rvert=5,\lvert\mathcal{A}\rvert=2,\gamma=0.9$），记录每轮 $\sum_s v_{\pi_k}(s)$：

```python
import numpy as np
rng = np.random.default_rng(11)          # 固定种子
nS, nA, g = 5, 2, 0.9
P = rng.random((nS, nA, nS)); P /= P.sum(2, keepdims=True)
R = rng.uniform(-1, 1, (nS, nA))

def evaluate(pol):                       # 迭代评估 → v_π
    V = np.zeros(nS)
    while True:
        d = 0.0
        for s in range(nS):
            v = R[s, pol[s]] + g * P[s, pol[s]] @ V
            d = max(d, abs(v - V[s])); V[s] = v
        if d < 1e-8: return V

pol = np.zeros(nS, int)                  # 初始：全选动作 0
for it in range(100):
    V = evaluate(pol)
    print(f"轮次 {it}: 策略 = {pol.tolist()}   Σv = {V.sum():+.4f}")
    new = (R + g * np.einsum('san,n->sa', P, V)).argmax(1)
    if np.array_equal(new, pol):
        print(f"策略稳定于第 {it} 轮 → 最优策略 {pol.tolist()}，Σv* = {V.sum():+.4f}")
        break
    pol = new

# 预期输出（Σv 单调非降，正是定理的数值体现）：
# 轮次 0: 策略 = [0, 0, 0, 0, 0]   Σv = -12.8126
# 轮次 1: 策略 = [1, 0, 0, 1, 1]   Σv = -9.7419
# 轮次 2: 策略 = [1, 0, 0, 0, 1]   Σv = -9.5101
# 策略稳定于第 2 轮 → 最优策略 [1, 0, 0, 0, 1]，Σv* = -9.5101
```

**小结**：PI 收敛由"单调（改进定理）+ 有限（策略空间有限）"保证；上界指数但实际轮数极少，是典型的"少而重"算法。

### 10.3 价值迭代（VI）的收敛性

**$\mathcal{T}^*$ 是 $\gamma$-收缩**。$(\mathcal{T}^* v)(s) = \max_a \sum_{s',r} P(s',r \mid s,a)[r + \gamma v(s')]$。对任意 $u,v$：

```
‖T*u − T*v‖_∞ = max_s | max_a f_u(a) − max_a f_v(a) |        （f_x(a) = Σ P[ r + γ x(s') ]）
             ≤ max_{s,a} | f_u(a) − f_v(a) |                 （|max f − max g| ≤ max|f − g|）
             = max_{s,a} | γ Σ_{s'} P(s'|s,a)(u−v)(s') | ≤ γ‖u − v‖_∞   □
```

**收敛性与误差界**：由巴拿赫不动点定理，$\mathcal{T}^*$ 在 $(\mathbb{R}^{\lvert\mathcal{S}\rvert}, \|\cdot\|_\infty)$ 上有唯一不动点 $v_*$；从任意 $v_0$ 出发，$v_{k+1} = \mathcal{T}^* v_k$ 收敛到 $v_*$，且

$$\|v_k - v_*\|_\infty \le \gamma^k \|v_0 - v_*\|_\infty$$

推导只有一行：$\|v_{k+1}-v_*\| = \|\mathcal{T}^* v_k - \mathcal{T}^* v_*\| \le \gamma\|v_k - v_*\|$，归纳即得。这是**线性（几何）收敛**，比率 $\gamma$；$\gamma \to 1$ 时急剧变慢。

**与策略评估的区别**：策略评估用**期望算子** $\mathcal{T}_\pi$（线性，不动点 $v_\pi$），VI 用**最优算子** $\mathcal{T}^*$（取 $\max$，非线性——$\max$ 引入的正是"最优"），不动点 $v_*$。前者回答"给定策略值多少"，后者回答"最优值多少"；两者同为 $\gamma$-收缩，证明同构。

**停机准则**（$\gamma=1$ 时失效）：以 $\|v_{k+1}-v_k\|_\infty < \theta$ 停机，则 $\|v_k - v_*\| < \theta/(1-\gamma)$。故 $\gamma$ 越接近 1，同等精度所需 $\theta$ 越小；$\gamma=1$ 时 $\mathcal{T}^*$ 不是收缩映射，$\gamma^k$ 界失效，GridWorld 能收敛依赖吸收结构（5.2 节水波扩散），无统一误差界。

**小结**：VI 收敛是纯泛函分析结论——$\mathcal{T}^*$ 的 $\gamma$-收缩性 + 巴拿赫定理给出全局收敛与几何误差界；与策略评估共享同一证明骨架，区别只在期望算子换成最优算子。

### 10.4 泛化：GPI 为什么"不完美也能收敛"

GPI 的评估把 $v$ 拉向 $v_\pi$，改进把 $\pi$ 拉向 $\mathrm{greedy}(v)$。几何上是两条目标线，迭代在两者间往复逼近交点：
...
```
    ...
    └────────────────────────────► v
    评估线：v = v_π
    ①③… 评估：竖直移动（固定 π），把 v 拉向 v_π
    ②④… 改进：水平移动（固定 v），把 π 拉向 greedy(v)
    折线逐圈逼近 ★；步长不要求为零，也不要求一次到位
```

**为什么不要求完全收敛**：收敛的充分条件远比"评估收敛、改进一步到位"宽松——

- **价值迭代**即"评估只做一步"的极端 GPI，收敛性由 $\mathcal{T}^*$ 收缩性独立保证（10.3）；
- **截断策略迭代**（每轮评估只做固定 $m\ge1$ 次扫描再贪心）对任意 $m$ 收敛：$m=1$ 即 VI，$m\to\infty$ 即 PI，同一谱系的两端；
- 只要每步**方向正确、步长非零、不越过目标**，折线就单调逼近交点。

**互相促进**：改进使评估目标线整体抬高（$v_{\pi'} \ge v_\pi$），评估使贪心基于更准确的价值——一方的输出总在改善另一方的输入，形成正反馈；策略有限、价值有界，杜绝发散。

> **核心思想**：GPI 的宽容性是"单调逼近 + 有界空间"的必然结果：两个不完备的过程只要方向正确，交替进行就收敛。这也是第 3 章以后一切采样式算法（评估永不收敛）仍能工作的理论底气。

**小结**：PI 与 VI 是 GPI 谱系的两个端点；收敛性来源于"方向正确"而非"一步到位"。

### 10.5 收敛速度对比

| 算法 | 每轮代价（单次扫描 $O(\lvert\mathcal{S}\rvert^2\lvert\mathcal{A}\rvert)$） | 典型轮数 | 收敛保证 / 误差界 | 适用场景 |
|------|------|------|------|------|
| 策略迭代 PI | 评估 $k$ 次扫描 + 改进：$O(k\lvert\mathcal{S}\rvert^2\lvert\mathcal{A}\rvert)$ | 少（3–10；上界 $\lvert\mathcal{A}\rvert^{\lvert\mathcal{S}\rvert}$） | 精确停机（策略不变即最优） | 状态空间小、需"每轮有完整可用策略" |
| 价值迭代 VI | 1 次扫描：$O(\lvert\mathcal{S}\rvert^2\lvert\mathcal{A}\rvert)$ | 多（$\approx$ 最长最短路径长度；$\gamma<1$ 时随 $\gamma\to1$ 急剧增多） | $\lVert v_k-v_*\rVert_\infty \le \gamma^k\lVert v_0-v_*\rVert_\infty$ | 大规模离散 MDP 默认选择 |
| 截断策略迭代（$m$ 次扫描） | $O(m\lvert\mathcal{S}\rvert^2\lvert\mathcal{A}\rvert)$ | 介于两者之间 | 线性收敛（误差界常数小于 VI） | 折中：每轮略贵、轮数明显减少 |

要点：**每轮代价** VI 最轻、PI 最重，截断 PI 用 $m$ 调节；**轮数** VI 与"信息从终止状态传播的速度"同阶（GridWorld 中 $k$ 轮价值恰覆盖 $k$ 步内可达区域），PI 轮数几乎与状态数无关（每轮全局修正）；**$\gamma$** 越接近 1，VI 的 $\gamma^k$ 衰减越慢、受影响最直接，PI 有精确停机判据、受影响较小。经验法则：模型已知且状态数上万默认 VI；状态数小或需中间策略用 PI；$m$ 取 3–10 的截断 PI 常取得最佳平衡。

**小结**：三者的取舍是"每轮代价 × 轮数"的权衡——VI 每轮便宜但要很多轮，PI 每轮贵但轮数极少，截断 PI 用 $m$ 在两端间插值。

### 10.6 理论小结与证明中的常见误区

**理论骨架**：策略改进定理（10.1）保证单调，策略空间有限（10.2）保证终止，$\mathcal{T}^*$ 收缩（10.3）保证几何收敛，GPI（10.4）证明"方向正确即可"。

**常见误区**：

1. **把定理的假设当结论**：假设是"逐状态 $q_\pi(s,\pi'(s)) \ge v_\pi(s)$"，结论是"$v_{\pi'} \ge v_\pi$"；反过来不成立，且假设必须对所有 $s$ 成立。
2. **误以为"贪心一次即最优"**：一次改进只保证"不差"（3.5 节例子：改进一次后状态 13 仍错），需多轮迭代。
3. **误以为 VI 的中间 $v_k$ 对应某个策略**：$v_k$ 一般不是任何策略的价值函数，只有极限 $v_*$ 才是。
4. **误以为 VI 需要好的初值**：收缩映射从任意 $v_0$ 收敛，初值只影响常数与轮数。
5. **误以为 $\gamma=1$ 时误差界照用**：此时 $\mathcal{T}^*$ 不是收缩映射，收敛依赖吸收结构，无统一误差界。
6. **混淆两个算子**：$\mathcal{T}_\pi$（线性，不动点 $v_\pi$）vs $\mathcal{T}^*$（非线性，不动点 $v_*$）；把 $\max$ 换成期望即退化为策略评估，反之即升级为 VI。

> **核心思想**：本章全部收敛性结论归结为三句话——**改进定理保证单调，有限空间保证终止，收缩算子保证几何收敛**。理解适用条件，比记住公式更重要。

**小结**：理论部分至此闭合——PI 的收敛是组合式的（单调 + 有限），VI 是分析式的（收缩 + 不动点），GPI 把两者统一为同一折线逼近过程；后续章节的采样式算法将沿用"方向正确即收敛"的论证框架。
