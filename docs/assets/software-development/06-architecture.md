...
<!-- 增补小节：追加到 06-architecture.md 第七节之后，不修改原文件。 -->

## 八、底层原理：分布式系统理论内核

> 前七节描述架构的"形状"，本节深入其"物理内核"：CAP/BASE 划定取舍边界，一致性哈希与 Raft 解决数据放置与共识，分布式事务与消息队列提供跨节点的一致性与异步协作原语——理解了这些，才能解释"架构为什么必须这样设计"。

### 8.1 CAP 定理

**形式化定义**：

- **一致性（Consistency）**：任何读操作都能读到最近一次写的结果。形式化为线性一致性：存在全局操作顺序，每个操作原子生效且与实时序相容。
- **可用性（Availability）**：每个到达非故障节点的请求最终都会收到非错误响应——"总能响应，但不保证最新"。
- **分区容错（Partition Tolerance）**：网络分区（消息丢失或延迟超阈值）发生时系统仍能对外服务。

**为什么三者不可兼得（证明思路）**：

```text
反证：假设存在同时满足 C、A、P 的系统。网络分区把节点分为 G1、G2：
  1. 客户端向 G1 写 v1 —— P 允许 G1 在分区内正常服务
  2. 客户端向 G2 读 —— A 要求 G2 必须响应
  3. G2 未收到 v1（分区隔离），只能返回旧值 v0
  4. v0 ≠ v1，违反 C —— 矛盾
⇒ P 不可放弃（分区必然存在），只能在 C 与 A 之间二选一。
```

**CP vs AP 实例对比**：

| 维度 | CP（分区时牺牲可用性） | AP（分区时牺牲一致性） |
|------|------------------------|------------------------|
| 分区时行为 | 拒绝读写，等待愈合 | 继续服务，容忍旧数据 |
| 典型实例 | ZooKeeper、etcd、HBase | Cassandra、DynamoDB、Eureka、Redis 异步复制 |
| 适用场景 | 元数据、配置、选主、账务 | 商品浏览、feed、会话缓存 |
| 恢复策略 | 愈合后同步日志 | 后台补偿 + 版本向量解决冲突 |

> **核心思想**：CAP 不是"三选二"的菜单，而是"P 必选、C/A 二选一"。真正的设计问题是：**分区发生时，优先保数据正确，还是优先保请求成功？**

**小结**：CAP 以反证法证明分区客观存在时一致性与可用性不可兼得，架构师必须按业务语义明确 CP/AP 定位。

### 8.2 BASE 理论与最终一致性

BASE（Basically Available, Soft state, Eventual consistency）是 AP 路线的工程化落地：

- **基本可用**：分区或过载时允许功能降级（限流、返回缓存），而非整体不可用。
- **软状态**：允许节点存在中间态，不要求任何时刻全局一致。
- **最终一致**：无新写入的静默期后，所有副本经异步收敛达成一致。收敛强度分级：因果一致性 > 读己之写 > 单调读 > 弱一致。

**事件溯源（Event Sourcing）**：不保存"当前状态"，只追加存储**事实（事件）**，状态由事件流重放：

```text
写路径: Command ──▶ 校验执行 ──▶ 追加事件（append-only EventStore）
读路径: 状态 = fold(事件流, 初始值)；快照用于截断重放
```

优势：完整审计轨迹、可回溯任意历史状态、事件是唯一事实源；代价：事件流无限增长需快照压缩，读模型需另行投影。

**CQRS（命令查询职责分离）**：写模型（Command）与读模型（Query）分离为独立存储；与事件溯源组合时读模型由事件异步投影生成，读写最终一致：
...
> **核心思想**：BASE 用"最终一致"换"持续可用"；事件溯源 + CQRS 把"状态"降级为"事件的投影"，允许读写异步收敛——这是高并发系统的标准底层一致性模型。

**小结**：BASE 把一致性从强约束降为可收敛的弱约束；事件溯源提供事实源，CQRS 隔离读写模型，二者组合是最终一致性架构的标准实现路径。

### 8.3 一致性哈希（Consistent Hashing）

**为什么需要**：普通取模 `node = hash(key) % N` 在节点数 N 变化时约 N/(N+1) 的 key 迁移，缓存场景等同集体失效（雪崩）。

**哈希环**：key 与节点都哈希到 [0, 2³²) 的环上，key 顺时针遇到的第一个节点即归属：
...
环上按 hash 顺时针排列（A < B < C）：
  k1 在 A 与 B 之间  ⇒ 顺时针首个节点 = node B
  k2 在 C 与 A 之间(回绕) ⇒ 归属 node A
```

**虚拟节点**：每个物理节点在环上放置 K 个虚拟节点（`nodeA#0..#K-1`），解决节点少时分布不均、增减时负载转移不均两个问题。K 越大越均匀但路由表越大（工程上 K 取 100~200）。

**与普通哈希对比**：

| 维度 | 取模哈希 | 一致性哈希 |
|------|----------|-----------|
| 增减节点迁移量 | 约 N/(N+1) 的 key | 仅约 1/N 的 key |
| 数据倾斜 | 天然均匀 | 需虚拟节点矫正 |
| 路由复杂度 | O(1) | O(log N)（环上二分） |
| 典型应用 | 单机分库分表 | Memcached、Cassandra、Kafka 分区分配 |
...
class ConsistentHash:
    def __init__(self, nodes, vnodes=100):
        self.ring = {}                      # hash -> 物理节点
        for node in nodes:
            for i in range(vnodes):         # 每物理节点 vnodes 个虚拟节点
    ...
    def get_node(self, key):
        ...
        for kh in self.sorted_keys:         # 量大时改用 bisect
            if kh >= h:
                return self.ring[kh]
        return self.ring[self.sorted_keys[0]]   # 回绕环起点

ch = ConsistentHash(["nodeA", "nodeB", "nodeC"])
dist = {}
for i in range(1000):
    n = ch.get_node(f"user_{i}")
    dist[n] = dist.get(n, 0) + 1
print(dist)
# 预期输出（虚拟节点下近似均匀，1000 个 key 时偏差 < 5%，key 越多越收敛）:
# {'nodeA': 343, 'nodeB': 345, 'nodeC': 312}
```

> **核心思想**：一致性哈希把"节点数参与哈希运算"改为"节点在环上占位"，使映射局部化——增删节点只影响环上相邻区间，迁移量从 O(N) 降为 O(1/N)。

**小结**：一致性哈希以环状寻址 + 虚拟节点，在 O(log N) 路由代价下把节点变更迁移量降至最低，是缓存与存储集群弹性伸缩的基石。

### 8.4 Raft 共识算法

Raft 将共识分解为三个子问题：领导者选举、日志复制、安全性。节点三态：
...
**领导者选举（term 与心跳）**：时间划分为递增的任期 term；Follower 在选举超时内未收到 Leader 心跳，即自增 term 转为 Candidate 发起投票，获得多数（N/2+1）票者当选。term 用于隔离过期 Leader 的写入。
...
Node1(Follower)  Node2(Follower)  Node3(Follower)
      ...
      │◀──Vote(term=1)──│                 │
      │◀──Vote(term=1)───────────────────│
      │  2 票(含自己) > 半数 → 当选 Leader
      │──AppendEntries(心跳, term=1)──▶│
      │──AppendEntries(心跳, term=1)──────▶│
```
...
```text
Client ──▶ Leader: 追加条目 index=1
Leader ──▶ 多数 Follower: AppendEntries(log=1)
多数确认后: Leader 更新 commitIndex=1 → 应用状态机 → 回复 Client
```

- **提交规则**：条目被复制到**大多数节点**即视为已提交；已提交条目绝不允许被覆盖。
- **安全性保证**：① 选举限制——Candidate 日志必须"不旧于"多数节点（比较最后条目的 term 与 index）才能当选；② 日志匹配——AppendEntries 携带 prevLogIndex/prevLogTerm，不一致则拒绝并回退，保证日志前缀一致；③ 只有 Leader 能提交，保证已提交条目的顺序与幂等性。

**选举伪代码与预期输出**：

```python
def run_election(node, peers, current_term):
    current_term += 1                  # 任期自增，隔离旧 Leader
    node.state = "candidate"
    votes = {node.id}                  # 先投自己
    for peer in peers:                 # 并发请求投票（携带 term 与最后日志）
        if request_vote(peer, term=current_term, last_log=node.last_log):
            votes.add(peer.id)
    if len(votes) > len(peers) // 2:   # 严格多数（N=3 需 2 票）
        node.state = "leader"
        broadcast_heartbeat(term=current_term)
        return "LEADER"
    return "FOLLOWER"                  # 收到更高 term 或超时未过半则退回

# 预期输出（3 节点、无丢包、日志同样新）:
# term=1 选举超时 → 收到 2 票(含自己) → 2 > 3//2=1 → 返回 LEADER
```

> **核心思想**：Raft 用"多数派"化解不确定性——任意两个多数集合必相交，Leader 一旦被多数节点确认，其日志条目不可能被后续任期覆盖，共识由此可证明地收敛。

**小结**：Raft 通过任期号 + 多数派投票 + 日志匹配约束，把共识收敛为确定性可证明安全的算法，是 etcd、Consul、TiKV 的共识底座。

### 8.5 分布式事务：2PC / 3PC / Saga / TCC

**2PC（两阶段提交）**：协调者先向所有参与者发 prepare，全部 yes 后广播 commit，否则 abort。
**3PC**：在 2PC 前增加 canCommit，并把提交拆为 preCommit + doCommit，缩小协调者故障时的阻塞窗口。
**Saga**：长事务拆为本地事务序列 T₁..Tₙ，每个 Ti 配补偿事务 Ci，任一步失败则逆序补偿。
**TCC**：业务方实现 Try（预留资源）/ Confirm（确认）/ Cancel（释放），由框架编排。
...
| 隔离性 | 有（全局锁） | 有 | 无（业务自理） | 预留资源近似 |
| 协调者故障 | 参与者阻塞 | 超时可自决 | 无单点 | 依赖框架恢复 |
| 业务侵入 | 低 | 低 | 中（补偿逻辑） | 高（三方法改造） |
| 吞吐 | 低（两轮 RTT+锁） | 低 | 高 | 中 |
| 适用 | 短事务、同构存储 | 理论优、落地少 | 跨服务长流程 | 资金类强校验 |

**各自优缺点**：

- **2PC**：强一致、实现简单；但协调者单点、参与者同步阻塞、宕机后悬挂事务无法自动处理。
- **3PC**：阻塞窗口小；但分区下参与者超时提交而协调者已 abort 仍可能不一致，实际部署极少。
- **Saga**：高吞吐、无全局锁；但缺隔离性，需语义锁/幂等，正向与补偿必须可重入。
- **TCC**：业务级资源预留、可控性强；但开发量大，空回滚与悬挂需幂等表 + 事务控制表处理。

**Seata AT 模式原理**：AT（Auto Transaction）沿 2PC 思想演进，对业务无侵入：

```text
① 业务 SQL 前：AT 代理生成 before image
② 执行业务 SQL：生成 after image，与 before 一并写入 undo_log（同库同事务）
③ 全局提交：异步删除 undo_log
④ 全局回滚：按 undo_log 逆序补偿还原为 before image；
   冲突检测：当前数据与 after image 不一致则人工/重试介入
```

> **核心思想**：强一致事务的代价是阻塞与低吞吐；Saga/TCC 用"补偿"与"预留"换取最终一致与高并发。选型本质是回答：**这笔业务能接受多长时间的账不平？**

**小结**：2PC/3PC 保强一致但阻塞低吞吐；Saga 与 TCC 以补偿和预留换取最终一致；Seata AT 用镜像日志自动化补偿，是 2PC 思想的无侵入工程化实现。

### 8.6 消息队列底层模型（以 Kafka 为例）

**partition / offset / consumer group**：Topic 划分为多个 partition（并行度与有序性边界），消息在 partition 内拥有单调递增的 offset；同一 group 内的消费者共同消费一个 topic，每个 partition 同一时刻只归组内一个消费者（组内有序）；不同 group 独立推进 offset（广播语义）：

```text
Topic "orders"（3 个 partition，append-only 日志）
┌───────────┐ ┌───────────┐ ┌───────────┐
│  P0       │ │  P1       │ │  P2       │
...
└───────────┘ └───────────┘ └───────────┘
     │              │            │
     ▼              ▼            ▼
Group-A:  C1 ── C2      C3        ← 组内分担，partition 只归一个消费者
Group-B:  C4 ── C5 ── C6          ← 组间 offset 独立（订阅/广播）
```

**日志追加写与顺序读**：存储本质是 partition 上的 append-only 日志（segment 段文件 + 稀疏索引）。生产者只追加（顺序写，命中页缓存、批量发送），消费者按 offset 顺序读，配合零拷贝（sendfile）实现高吞吐：

```text
Producer ──追加──▶ Partition 日志（顺序写） ──顺序读──▶ Consumer
                    segment: .log + .index（稀疏索引）
```

**投递语义**：

- **at-least-once**：先处理再提交 offset；崩溃后重复消费，要求消费者幂等。
- **at-most-once**：先提交 offset 再处理；可能丢消息。
- **exactly-once**：幂等生产者（PID+序号去重）+ 跨 partition 事务原子提交 + 消费者事务性读取；代价是吞吐下降，通常在账务场景启用。

> **核心思想**：消息队列的高吞吐来自"把随机读写变为顺序读写、把一致性责任下放给消费者"。partition 是并行与有序的折中单元，offset 是进度指针，语义选择本质是吞吐与重复/丢失风险的权衡。

**小结**：Kafka 以 partition 承载并行度、offset 记录进度、group 隔离消费视图，用追加写与顺序读逼近磁盘极限吞吐；at-least-once 是默认选择，exactly-once 需幂等与事务机制叠加。
