...
> 本节深入内核：容器如何隔离、镜像如何分层、Docker 如何构建、K8s 如何调度与组网、流水线如何执行。理解原理，排障与架构设计才能直达本质。

### 8.1 容器隔离原理：namespace 与 cgroup

容器不是轻量级虚拟机，而是一组受限的普通进程，隔离由内核两套机制提供：

- **namespace**：视图隔离——让进程"看不见"宿主机其他资源。常用 7 种：PID（进程号，容器内 PID 1 是主进程）、NET（网卡/路由）、MNT（挂载点）、UTS（主机名）、IPC（消息队列）、USER（用户 ID 映射）、CGROUP（cgroup 视图）。
- **cgroup**：资源限制——约束进程"能用多少"CPU/内存/IO。运行时（runc）以 `clone()` 传入 namespace 标志创建进程，再把 PID 写入 cgroup 子系统。

```
宿主机: PID 1 systemd | PID 200 容器进程 | 网络栈 eth0
容器内: PID 1 主进程 | PID 2 子进程 | 看不到宿主机进程 (PID 重新编号)
```
...
**可运行示例**——cgroup v2 限制 CPU 为 0.5 核：

```bash
sudo mkdir /sys/fs/cgroup/cpu.demo
echo "50000" | sudo tee /sys/fs/cgroup/cpu.demo/cpu.max   # 50ms/100ms 周期
echo $$ | sudo tee /sys/fs/cgroup/cpu.demo/cgroup.procs   # 本 shell 入组
# 预期输出: 子进程合计最多占 50% CPU (stress-ng --cpu 4 验证)
```

> **核心思想**：容器 = 普通进程 + namespace（隔离视图）+ cgroup（资源限制）。

**小结**：namespace 决定"看得见什么"，cgroup 决定"能用多少"。

### 8.2 镜像分层原理：UnionFS 与写时复制

镜像由只读层（layer）堆叠，运行时叠加可写层，底层是联合文件系统（UnionFS），Docker 默认用 **OverlayFS** 合并多层视图：

```
容器可写层(读写发生于此) -> 层4: CMD -> 层3: COPY -> 层2: RUN build -> 层1: FROM
                            ↑ 镜像只读层 (Dockerfile 指令逐层生成)
```

**写时复制（CoW）**：读文件按"上层优先"逐层查找；写文件先把下层文件复制到可写层再修改，只读层永不变。故多容器共享全部只读层，仅各自维护薄薄的可写层，磁盘占用极低。

**Dockerfile 每行指令如何产生层**：`FROM`/`RUN`/`COPY`/`ADD` 各创建一层（`ENV`/`CMD` 只写元数据）。**层越多镜像越大**：① 被删文件仍留在下层——apt 缓存即使 `rm` 掉也占空间；② 每层有元数据开销且无法跨层压缩。实践：合并 RUN、多阶段构建、`&& rm -rf /var/lib/apt/lists/*`。

> **核心思想**：分层 + CoW 使"共享"成为可能——镜像只存一份，分发与启动因此变快。

**小结**：层是存储单元，CoW 是共享与提速机制，控制层数即控制镜像体积。

### 8.3 Docker 构建原理：上下文、缓存与多阶段

**Build 上下文**：`docker build .` 的 `.` 指构建上下文——客户端将其打包发给守护进程，`COPY`/`ADD` 只能引用上下文内文件，故用 `.dockerignore` 排除 node_modules、.git 等。

**层缓存机制**：指令执行前先查缓存，命中"父层相同且指令相同"的层则跳过；**缓存失效（cache invalidation）**——任一前置层变化，其后所有层缓存失效，故"变动少的放前面"（先 `COPY go.mod go.sum` 再 `RUN go mod download`，最后 `COPY . .` 与 `RUN go build`，源码变更只重建后两层）。

**多阶段构建**：单文件多个 `FROM`，前一阶段产物经 `COPY --from` 拷入后一阶段；构建阶段保留工具链，运行阶段只含产物，最终镜像只含最后阶段的层：

```dockerfile
FROM golang:1.22 AS builder        # 阶段1: 完整工具链
RUN go build -o /server .
FROM alpine:3.19                   # 阶段2: 精简运行时
COPY --from=builder /server /server
ENTRYPOINT ["/server"]
# 预期输出: 最终镜像约 10MB (对比单阶段约 300MB)
```

> **核心思想**：构建的本质是"可复现的层序列"；缓存让重复构建变快，多阶段让产物变小。

**小结**：上下文界定输入范围，缓存决定构建速度，多阶段决定产物体积。

### 8.4 Kubernetes 调度原理：控制面、调度器与控制器

**架构**：控制面——**kube-apiserver**（唯一入口，统一读写状态）、**etcd**（键值存储，状态唯一事实源）、**kube-scheduler**（选节点）、**kube-controller-manager**（运行控制器）；数据面——**kubelet**（节点 agent，创建/销毁容器并上报）、**kube-proxy**（维护转发规则）、**containerd**（运行容器）。kubelet 经 apiserver 的 watch 机制获知任务。

**Pod 调度流程（过滤-打分）**：① **过滤**：剔除资源不足、端口冲突、亲和性不匹配、未容忍 taint 的节点；② **打分**：按资源余量、亲和性、拓扑分布评分取最高；③ 绑定结果写入 API Server，kubelet 监听到后创建 Pod。

**Controller 的声明式循环（Reconcile Loop）**：提交期望状态（如 `replicas: 3`），控制器持续"观察当前 → 对比期望 → 施加变更"直至一致：

```
期望(3) -> 观察(当前1) -> 差异=2 -> 创建2个Pod -> 再观察… (一致后空转)
```

**kube-proxy 与 Service**：Service 是虚拟 IP（ClusterIP），无实体；kube-proxy 监听 Service/Endpoints 变化，写入 iptables/IPVS 规则把 ClusterIP 流量负载均衡到后端 Pod。Pod 重建 IP 变化而 Service 不变，实现解耦。

> **核心思想**：K8s 的一切都是"期望状态 + 控制器循环"，自愈的本质是控制器把现实拉回期望。

**小结**：apiserver+etcd 是大脑，调度器负责"放哪"，controller 负责"维持"，kube-proxy 负责"转发"。

### 8.5 容器网络：CNI、Pod 模型与 Service 寻址

**CNI（Container Network Interface）**：容器运行时与网络插件的标准接口；kubelet 创建 Pod 时调用 CNI 插件（calico/flannel/cilium）分配 IP、创建虚拟网卡并接入网络。

**bridge 与 overlay**：bridge 在宿主机建网桥（cni0），veth 接入，同机互通（Docker 默认）；overlay 用 VXLAN 隧道封装，跨宿主机组成虚拟二层网络（K8s 集群跨节点）。

**Pod 网络模型**：每个 Pod 拥有独立 IP，**Pod 间无需 NAT 直接互通**（扁平网络）；Pod 内多容器共享同一网络命名空间（共享 IP 与 localhost），经 localhost 互访。

**Service ClusterIP 与 DNS**：ClusterIP 是集群内虚拟 IP，由 kube-proxy 转译为 Pod IP；CoreDNS 提供域名 `<svc>.<ns>.svc.cluster.local`，Pod 内按服务名访问即可（`nslookup my-service.default.svc.cluster.local` 预期解析出 ClusterIP，DNS 服务器 10.96.0.10）。

> **核心思想**：K8s 网络把"IP 会变"的 Pod 封装为"名字稳定"的 Service，DNS + kube-proxy 是这层抽象的实现。

**小结**：CNI 解决"怎么连通"，Pod 模型解决"以什么为单位连通"，Service+DNS 解决"连到哪个稳定入口"。

### 8.6 CI/CD 原理：DAG、制品与 GitOps

**Pipeline 的 DAG 执行模型**：任务间依赖构成有向无环图——无环保证不死锁，有向保证顺序；无依赖任务并行，有依赖者等前置完成：
...
**制品（Artifact）流转**：构建产物作为**不可变制品**带唯一标识（如镜像 digest）在阶段间传递：`提交 -> 构建出 myapp@sha256:abc123 -> 测试该 digest -> 部署该 digest -> 回滚到上一 digest`，保证"测试过的就是部署的"。

**GitOps 核心思想：声明式 + 控制器**——把 K8s 的 controller 模式推广到交付链：① **声明式**：Git 仓库是期望状态的唯一事实源（infra-as-code），变更走 PR；② **控制器**：Argo CD/Flux 持续比对"Git 中的期望"与"集群实际"，有偏差即自动收敛（或告警等确认）；③ 部署 = 合并 PR，回滚 = revert commit，审计 = git log。命令示例：`argocd app diff my-app`（预期输出 `Sync Status: Synced / OutOfSync`）。

> **核心思想**：GitOps = 用 Git 当控制面，用控制器当执行者。"无人值守部署"的本质是控制器循环在自动工作。

**小结**：DAG 定义执行顺序，制品保证交付一致性，GitOps 把"部署"简化为"改代码"。

---

> **本节总结**：namespace+cgroup 与 UnionFS+CoW 是 Docker 的地基；过滤-打分调度、控制器循环、CNI 与 Service 抽象是 K8s 的骨架；DAG 与制品流转是 CI/CD 的引擎，GitOps 把声明式思想延伸至整个交付生命周期。
