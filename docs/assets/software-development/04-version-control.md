...
## 七、底层原理：Git 对象模型与合并算法

本节深入 Git 的存储与算法内核：对象模型回答"存了什么"，引用机制回答"分支是什么"，三路合并回答"冲突从哪来"。理解这三层，才能推断疑难问题：历史改写、悬空提交、仓库膨胀、诡异冲突。

### 7.1 对象模型与内容寻址

四种对象统一以 `<type> <size>\0<content>` 序列化后取 SHA-1 作为地址。**对象一经写入即不可变**，内容变化即产生新地址。一次 commit 在对象库中的完整对象 DAG：

```
tag "v1.0.0"（可选，含签名）
     │
     ▼
commit C1 ──► tree T1 ──┬──► blob B1   "hello.txt": "hello world"
   │  parent: 无         └──► tree T2 ──► blob B2   "main.go": "package main"
   │  author/committer/message
```

**核心思想**：Git 是内容寻址文件系统——`key = SHA-1(content)`，地址由内容决定而非路径，自动获得三项性质：

| 性质 | 传统路径寻址 FS | Git 内容寻址 |
|------|----------------|--------------|
| 定位方式 | 路径 → 可变内容 | 哈希 → 不可变内容 |
| 相同内容 | 重复存储 | 共享同一对象，天然去重 |
| 完整性 | 需额外校验 | 读取即校验，篡改必现 |

验证哈希公式（对象头 `blob 12` = 类型 + 内容字节数）：

```bash
$ printf 'hello world\n' | git hash-object --stdin
3b18e512dba79e4c8300dd08aeb37f8e728b8dad
$ printf 'blob 12\0hello world\n' | sha1sum
3b18e512dba79e4c8300dd08aeb37f8e728b8dad  -   # 两式一致，公式成立
```

**小结**：commit→tree→blob/tag 构成 DAG；内容寻址带来去重、不可变与自校验，是 Git 一切能力的地基。

### 7.2 引用与 HEAD

引用（ref）是"名字 → 哈希"的映射，只是 `.git/refs/` 下的普通文本文件：

```
.git/HEAD ──────────► "ref: refs/heads/main"   （符号引用，间接层）
                             │
.git/refs/heads/main ──► 1ba064af6d12e467b8622eda06d8d291c0a816b7 ──► commit C1
```

- **refs/heads/\***：分支，可变指针，随提交自动前移；**refs/tags/\***：标签，默认不移动；**HEAD**：符号引用（symbolic ref），存分支名而非哈希——"检出分支"的本质是改一行文本，不复制任何文件。

**detached HEAD**：`git checkout <hash>` 后 HEAD 直接存哈希，新提交无分支承接，成为悬空提交（dangling commit）。**reflog**（`.git/logs/HEAD`）以追加方式记录每次 HEAD 移动（旧/新哈希、操作者、动作），是引用的操作日志与恢复的最后防线：`git reflog` 查看，`git reset --hard HEAD@{1}` 回退，条目默认 90 天过期。示例：`git checkout 1ba064a && git commit -m "wip"` 后切回 main，wip 即悬空，但 reflog 中 `HEAD@{0}` 仍可找回。

**小结**：分支与标签都是"哈希的文本文件"，HEAD 是符号引用；detached HEAD 使提交脱离分支，reflog 保留引用移动轨迹，使误操作可逆。

### 7.3 三路合并算法（three-way merge）

合并不是"比较两份代码"，而是三方比对：**base**（两分支最近公共祖先 merge-base）、**ours**（HEAD）、**theirs**（被合入方）。逐文件逐块决策：
...
**核心思想**：三路合并比较 base 与两边的"差异"而非盲目覆盖——双方各自修改不同区域可自动合并；只有同区域都改动（base→B 且 base→C）才冲突。冲突即"合并算法无法生成唯一 tree"，中止并留下标记：
...
合并提交 M 与普通提交无异，仅 parent 列表含两个哈希（`git log --graph` 的分叉线即由此而来）：

```
    o─── A (ours, HEAD)
   /     \
  o (base) M ← 合并提交，parent = [A, B]
   \     /
    o─── B (theirs)
```

**小结**：三路合并以 merge-base 为锚，单方改动自动合并、双方同区改动才冲突；合并提交即双亲 commit，DAG 因此分叉交汇。

### 7.4 分支的本质：指针、快进与 cherry-pick

分支不是副本，只是指向 commit 的可变指针。创建分支 = 复制一个 41 字节文本文件，成本 O(1)：`git branch feature/x` 等价于 `cp .git/refs/heads/main .git/refs/heads/feature/x`。

**快进合并 vs 三方合并**——由祖先关系决定，而非用户选择：

```
快进：theirs 是 ours 的后代，指针直接前移，零新对象
  main: o──o──o ──► 合并后直接指向 feat 顶端（无合并提交）
  feat:         └──o──o
三方：两分支分叉，需 merge-base，产生双亲合并提交 M
  main: o──o──o──M
  feat:     └──o──o
```

**cherry-pick 对象层面**：对目标 C 与其父 P 求 diff（`diff(P, C)`），把补丁应用到当前 HEAD 并生成**全新提交**——新哈希、新时间戳、新 parent。即"重放变更"而非"搬运提交"：`git cherry-pick 1ba064a` 的对象操作是 `diff(1ba064a^, 1ba064a)` → 应用 → 新 commit（哈希 ≠ 1ba064a）。快进合并不产生新提交（仅移指针，DAG 线性）；三方合并产生双亲提交 M（DAG 分叉交汇）；变基则重写提交使历史线性化。

**小结**：分支是 O(1) 指针操作；能否快进取决于祖先关系；cherry-pick 是"重放 diff 生成新提交"，故可跨分支移植单次变更。

### 7.5 Plumbing 与 Porcelain：手动创建一次 commit

**porcelain**（`commit`/`checkout`/`merge`）是面向人的高层封装；**plumbing**（`hash-object`/`write-tree`/`commit-tree`/`update-ref`）是 Git 自身与脚本使用的底层原语。一次 `git commit` 实际是四步：写 blob → 写 tree → 写 commit → 移引用。用纯 plumbing 复现（真实输出）：
...
$ git commit-tree 68aba62e560c0ebc3396e8ae9335232cd93a3f60 -m "first commit"
1ba064af6d12e467b8622eda06d8d291c0a816b7    # commit 哈希随 author/时间戳变化，结构不变

$ git update-ref refs/heads/main 1ba064af6d12e467b8622eda06d8d291c0a816b7
$ git symbolic-ref HEAD refs/heads/main
```

**核心思想**：porcelain 只是上述四步的编排——`git commit`、`git branch`、`git tag` 分别对应 `commit-tree`、`update-ref refs/heads/*`、`update-ref refs/tags/*`。排查"提交去哪了""引用为何移动"时，直接读 `.git/refs/` 与 `git cat-file -p <hash>`。

**小结**：提交 = 写三个对象 + 移一个指针；porcelain 是编排层，plumbing 是事实层，这是 Git 可脚本化、可审计的根源。

### 7.6 对象打包与 GC

松散对象（loose object）一对象一文件，zlib 压缩。对象多了以后 inode 爆炸、传输低效。**packfile** 将对象拼入单个 `.pack` 文件，配 `.idx` 索引（哈希 → 包内偏移），并做 **delta 压缩**：相似对象（如相邻版本同一文件）排序后，后者只存"对前者的差异"，形成 delta 链，仅保留一个完整基线。

```bash
$ git count-objects -v          # gc 前：3 个松散对象
count: 3
in-pack: 0
packs: 0

$ git gc                        # 打包 + 清理不可达对象 + 修剪过期 reflog
$ git count-objects -v          # gc 后：全部入包，寻址方式不变，读取透明
count: 0
in-pack: 3
packs: 1
size-pack: 1
...
注意：`git gc` 只删除**不可达**对象（悬空提交、孤儿 blob），被任何引用（含 reflog）可达的对象永远安全。

**小结**：packfile 以"一文件多对象 + delta 差异链"解决对象膨胀，索引保证寻址透明；GC 打包、清理、修剪三步控制仓库体积，且永不触碰可达历史。
